...
if ($share && !$token) {
    ...
    $stmt = $db->prepare('INSERT INTO export_tokens (token, project_id, format, from_date, to_date, expires_at) VALUES (?, ?, ?, ?, ?, ?)');
    $stmt->execute([$shareToken, $project['id'], $format, $from, $to, $expires]);
    $baseUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http') . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost');
    $scriptPath = $_SERVER['SCRIPT_NAME'] ?? '/api/export.php';
    $shareUrl = $baseUrl . $scriptPath . '?token=' . $shareToken;
